    ...
    <div className="page-stack">
      ...
      {user?.role === 'admin' && (
        <div className="admin-command-strip">
          ...
          <Link to="/admin/validation" className="button button--secondary">Open validation queue <ArrowRight size={16} /></Link>
        </div>
      )}
          ...
          <SectionCard title="Critical Attention" subtitle="Highest priority monitoring signals.">
            <div className="attention-list">
              {[...areas].sort((a, b) => b.riskScore - a.riskScore).slice(0, 3).map((area) => (
                <Link to="/risk" key={area.id} className="attention-item">
      ...
      <div className="dashboard-bottom-grid">
        <SectionCard title="Recent Operational Activity" subtitle="Latest data and decisions across SIGMA." action={<Link to="/patrols" className="text-link">View all</Link>}>
        ...
        <SectionCard title="Operational Recommendations" subtitle="Priorities generated from risk, hotspot, and visibility gaps." action={<Link to="/risk" className="text-link">Review priorities</Link>}>
